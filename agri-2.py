...
with source_mode[0]:
    # L'appareil photo affiche "Clear" après capture, c'est natif à Streamlit
    uploaded_file = st.camera_input("Caméra Appareil")

with source_mode[1]:
    uploaded_file = st.file_uploader("Importer depuis la galerie", type=["jpg", "jpeg", "png"])

# ─── LOGIQUE CORRIGÉE : LE BOUTON APPARAÎT UNIQUEMENT SI UNE IMAGE EST PRÉSENTE ───
if uploaded_file is not None:
    image = Image.open(uploaded_file).convert('RGB')
    
    # Si l'image vient de la galerie, on l'affiche (st.camera_input l'affiche déjà tout seul)
    if not hasattr(uploaded_file, 'type') or uploaded_file.type != "image/jpeg":
         # Évite d'afficher deux fois la photo de la caméra
         if "camera_input" not in str(type(uploaded_file)):
             st.image(image, caption="Image sélectionnée", use_container_width=True)

    # LE BOUTON APPARAÎT JUSTE EN DESSOUS DE LA PHOTO CAPTURÉE OU CHOISIE
    st.write("")
